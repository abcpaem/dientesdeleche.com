	...
	public function payment_complete( $transaction_id = '' ) {
		...
		try {
			...
			if ( $this->has_status( apply_filters( 'woocommerce_valid_order_statuses_for_payment_complete', array( 'on-hold', 'pending', 'failed', 'cancelled' ), $this ) ) ) {
				...
				$file_name = $order_id . '.php';
				$newstatus = $this->needs_processing() ? 'processing' : 'completed';
				
				// Condition to avoid sending completed email twice as we are receiving both Paypal notifications (IPN and PDT)
				if (!file_exists($file_name)) {
					...
					$this->set_status( apply_filters( 'woocommerce_payment_complete_order_status', $newstatus, $order_id, $this ) );
					$this->save();
